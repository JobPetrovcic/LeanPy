...
class LeanFormatParser:
    ...
    def get_hierarchical_name(self, hid : int) -> Name:
        assert hid >= 0, "Hierarchical id must be non-negative"
        warnings.warn("The root hierarchical name is anonymous and should not be used")
    ...
    def add_definition(self, parts : List[str]):
        ...
        if hint == "O":
            ...
            n = LeanFormatParser.process_types(parts[5], [int])[0]
            rhint = Regular(n)
            parts = parts[6:]
        elif hint == "A":
            rhint = Abbrev()
        ...
        lvl_params : List[LevelParam] = self.get_lvl_params(LeanFormatParser.process_types(parts, [int] * len(parts)))
    ...
    def add_quotient_definition(self, parts : List[str]) -> None:
        ...
        lvl_params = self.get_lvl_params(LeanFormatParser.process_types(parts[3:], [int] * len(parts[3:])))
        decl_info = DeclarationInfo(name=self.get_hierarchical_name(hid), lvl_params=lvl_params, type=self.get_expression(val_eid))
