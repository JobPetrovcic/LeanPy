...
from LeanPy.Structures.Expression.Expression import App, Const, Expression, NatLit, StringLit
...
def reduce_bin_nat_pred(environment : Environment, op : Callable[[int, int], bool], arg1 : int, arg2 : int) -> Expression: 
    if op(arg1, arg2): return Const(name=environment.Bool_true_name, lvl_params=[])
    else: return Const(name=environment.Bool_false_name, lvl_params=[])
...
def nat_lit_to_constructor(environment : Environment, nat_lit : NatLit) -> Expression: # DOES NOT CHANGE ANYTHING
    ...
    if nat_lit.val == 0: return Const(name=environment.Nat_zero_name, lvl_params=[])
    return App(
        Const(name=environment.Nat_succ_name, lvl_params=[]),
...
def char_to_expression(environment : Environment, c : str) -> Expression: # DOES NOT CHANGE ANYTHING
    return App(
        Const(name=environment.Char_name, lvl_params=[]),
        NatLit(ord(c))
    )
...
def str_to_char_list(environment : Environment, s : str, ind : int = 0) -> Expression: # DOES NOT CHANGE ANYTHING
    ...
    if ind == len(s): 
        return App(
            Const(name=environment.List_nil_name, lvl_params=[environment.level_one]),
            Const(name=environment.Char_name, lvl_params=[])
        )
    else:
        return  App(
            App(
                App(
                    Const(name=environment.List_cons_name, lvl_params=[environment.level_one]),
                    Const(name=environment.Char_name, lvl_params=[])
                ),
...
@typechecked
def str_lit_to_constructor(environment : Environment, s : StringLit) -> Expression: # DOES NOT CHANGE ANYTHING
    char_list = str_to_char_list(environment, s.val, 0)
    return App(
        Const(name=environment.String_mk_name, lvl_params=[]),
        char_list
    )
