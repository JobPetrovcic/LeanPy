...
from typing import Any, Iterator, List, Sequence, Tuple, Dict

from typeguard import typechecked

from LeanPy.Structures.Environment.Declaration.Declaration import Axiom, Constructor, Declaration, DeclarationInfo, Definition, InductiveType, Opaque, Quot, RecursionRule, Recursor, Theorem
...
class LeanFormatParser:
    ...
    def process_line(self, line : str):
        ...
        # EXPRESSIONS
            ...
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_variable(parts)
        elif parts[1] == "#ES": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_sort(parts)
        elif parts[1] == "#EC": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_constant(parts)
        elif parts[1] == "#EA": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_apply(parts)
        elif parts[1] == "#EP": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_pi(parts)
        elif parts[1] == "#EL": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_lambda(parts)
        elif parts[1] == "#EZ" : 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_let(parts)
        elif parts[1] == "#EJ" : 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_proj(parts)
        elif parts[1] == "#ELN": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_nat_lit(parts)
        elif parts[1] == "#ELS": 
            count_tags[parts[1]] = count_tags.get(parts[1], 0) + 1
            self.add_expression_string_lit(parts)
    ...
    def from_file(file_path : str, **type_checker_args):
        with open(file_path, "r") as file:
            return LeanFormatParser(file, **type_checker_args)
    
    @staticmethod
    def load_type_checking_environment(file_path : str, **type_checker_args) -> Environment:
        parsed = LeanFormatParser.from_file(file_path, **type_checker_args)
        return parsed.type_checker.environment
