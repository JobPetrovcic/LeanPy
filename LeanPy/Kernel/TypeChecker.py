...
class TypeChecker:
    ...
    def cheap_beta_reduce(self, e : Expression) -> Expression:
        ...
        if not fn.has_loose_bvars:
            ...
            if i <= fn.db_index:
                raise UnboundVariableError(f"Unbound variable in the body of the lambda.")
    ...
    def infer_proj(self, proj : Proj, infer_only : bool) -> Expression:
        ...
        if not isinstance(inductive_fn, Const):
            raise ProjectionError(f"Expected a constant when unfolding the struct type for projection but got {inductive_fn.__class__.__name__}")
        ...
        if not isinstance(I_decl, Inductive):
            raise ProjectionError(f"Expected an inductive type when unfolding the struct type for projection but got {I_decl.__class__.__name__}")
        ...
        for i in range(I_decl.num_params):
            ...
            if not isinstance(r, Pi):
                raise ProjectionError(f"Expected a Pi type when instantiating projection args but got {r.__class__.__name__}")
        ...
        for i in range(proj.index):
            r = self.whnf(r)
            if not isinstance(r, Pi):
                raise ProjectionError(f"Expected a Pi type when reducing projection indices but got {r.__class__.__name__}")
        ...
        r = self.whnf(r)
        if not isinstance(r, Pi):
            raise ProjectionError(f"Expected a Pi type for projection index but got {r.__class__.__name__}")
