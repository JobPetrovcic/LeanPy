...
# IMPORTANT: The input to the Lean4ExportParser must be generated by the modified Lean4export tool available at https://github.com/ammkrn/lean4export @ format2024 since the official Lean4export tool does NOT export all necessary information.
...
class Lean4ExportParser:
    ...
    def read_multiple_input(self, le : int, parts : List[str], expected_type : type) -> Tuple[List[Any], List[str]]:
        ...
        if le == 0:
            ...
            return [], parts[1:]
        pt = Lean4ExportParser.process_types(parts[:le], [expected_type] * le)
        return pt, parts[le:]
    
    def read_all(self, parts : List[str], expected_type : type) -> List[Any]:
        return Lean4ExportParser.process_types(parts, [expected_type] * len(parts))
    ...
    def process_namespace_name(self, parts : List[str]):
        ...
        hid, _, hp_hid, name= Lean4ExportParser.process_types(parts, [int, str, int, str])
        self.add_hierarchical_name(hp_hid, hid, name)

    def process_namespace_id(self, parts : List[str]):
        self.assert_parts_tag(parts, "#NI", 1)

        hid, _, hp_hid, identifier = Lean4ExportParser.process_types(parts, [int, str, int, int])
    ...
    def add_universe_successor(self, parts : List[str]):
        ...
        uid, _, up_id = Lean4ExportParser.process_types(parts, [int, str, int])
    ...
    def add_universe_max(self, parts : List[str]):
        ...
        uid, _, up_id1, up_id2 = Lean4ExportParser.process_types(parts, [int, str, int, int])
    ...
    def add_universe_parameter(self, parts : List[str]):
        ...
        uid, _, hid = Lean4ExportParser.process_types(parts, [int, str, int])
    ...
    def add_expression_variable(self, parts : List[str]):
        ...
        eid, _, vid = Lean4ExportParser.process_types(parts, [int, str, int])
    ...
    def add_expression_sort(self, parts : List[str]):
        ...
        eid, _, uid = Lean4ExportParser.process_types(parts, [int, str, int])
    ...
    def add_expression_constant(self, parts : List[str]):
        ...
        pt = Lean4ExportParser.process_types(parts[:3], expected_types)
        parts = parts[3:]
        eid, _, hid = pt
    ...
    def add_expression_apply(self, parts : List[str]):
        ...
        eid, _, eid1, eid2 = Lean4ExportParser.process_types(parts, [int, str, int, int]) # info is not used for typechecking, only for pretty printing
    ...
    def add_expression_lambda(self, parts : List[str]):
        ...
        eid, _, _info, hid, eid1, eid2 = Lean4ExportParser.process_types(parts, [int, str, str, int, int, int]) # info is not used for typechecking, only for pretty printing
    ...
    def add_expression_let(self, parts : List[str]):
        ...
        eid, _, hid, eid1, eid2, eid3 = Lean4ExportParser.process_types(parts, [int, str, int, int, int, int])
    ...
    def add_expression_proj(self, parts : List[str]):
        ...
        eid, _, hid, index, expr = Lean4ExportParser.process_types(parts, [int, str, int, int, int])
    ...
    def add_expression_nat_lit(self, parts : List[str]):
        ...
        eid, _, val = Lean4ExportParser.process_types(parts, [int, str, int])
    ...
    def add_expression_string_lit(self, parts : List[str]):
        ...
        eid, _ = Lean4ExportParser.process_types(parts[:2], [int, str])
        parts = parts[2:]
        # the rest are hexadecimal values of utf-8 characters
        
        types = [str] * len(parts)
        pt : List[str] = Lean4ExportParser.process_types(parts, types)
    ...
    def add_definition(self, parts : List[str]):
        ...
        _, hid, val_type, val_eid, hint = Lean4ExportParser.process_types(parts[:5], types)
        parts = parts[5:]
        ...
        if hint == "O":
            ...
            n = Lean4ExportParser.process_types(parts[:1], [int])[0]
            rhint = Regular(n)
            parts = parts[1:]
        elif hint == "A":
            rhint = Abbrev()
    ...
    def add_theorem(self, parts : List[str]):
        ...
        _, hid, val_type, val_eid = Lean4ExportParser.process_types(parts[:4], types)
        parts = parts[4:]
    ...
    def add_axiomatic_definition(self, parts : List[str]):
        ...
        types = [str, int, int]
        _, hid, val_type = Lean4ExportParser.process_types(parts[:3], types)
        parts = parts[3:]
    ...
    def add_inductive_definition(self, parts : List[str]):
        ...
        _, hid, val_eid, is_rec, _is_nested, num_params, num_indices, num_ind_name_idxs = Lean4ExportParser.process_types(parts[0:8], types)
        ...
        ind_name_pt = Lean4ExportParser.process_types(parts[0 : num_ind_name_idxs], ind_name_types)
        ...
        ctor_num_type = [int]
        num_ctor_pt = Lean4ExportParser.process_types(parts[0:1], ctor_num_type)
    ...
    def add_quotient_definition(self, parts : List[str]) -> None:
        ...
        types = [str, int, int]
        _, hid, val_eid = Lean4ExportParser.process_types(parts[:3], types)
        parts = parts[3:]
    ...
    def add_constructor_definition(self, parts : List[str]):
        ...
        _, hid, val_eid, induct_hid, cidx, num_params, num_fields = Lean4ExportParser.process_types(parts[:7], types)
        parts = parts[7:]

        # the rest are hierarchical ids for the level params
        lvl_params = self.get_lvl_params(Lean4ExportParser.process_types(parts, [int] * len(parts)))
    ...
    def add_recursor_rule_definition(self, parts : List[str]):
        ...
        rid, _, hid, num_fields, rhs_eid = Lean4ExportParser.process_types(parts, [int, str, int, int, int])
    ...
    def add_recursor_definition(self, parts : List[str]):
        ...
        types = [str, int, int, int]
        _, hid, val_type, num_ind_name_idxs = Lean4ExportParser.process_types(parts[:4], types)
        parts = parts[4:]
        ...
        num_params, num_indices, num_motives, num_minors, num_rule_idxs = Lean4ExportParser.process_types(parts[:5], [int] * 5)
        parts = parts[5:]

        rule_idxs, parts = self.read_multiple_input(num_rule_idxs, parts, int)
        
        isK = Lean4ExportParser.process_types(parts[:1], [bool])[0]
        parts = parts[1:]
    ...
    def from_file(file_path : str):
        with open(file_path, "r") as file:
            return Lean4ExportParser(file)
    
    @staticmethod
    def load_type_checking_environment(file_path : str) -> TypeChecker:
        parsed = Lean4ExportParser.from_file(file_path)
