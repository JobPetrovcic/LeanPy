...
def replace_expression_aux(
    ...
    if expr in replace_cache:
        return replace_cache[expr]
    ...
    if new_expr is None:
        if isinstance(expr, BVar):
            ...
            r_fn = replace_expression_aux(expr.fn, fn, replace_cache, func_name)
            r_arg = replace_expression_aux(expr.arg, fn, replace_cache, func_name)
            if expr.fn is r_fn and expr.arg is r_arg:
                new_expr = expr
            else:
                new_expr = App(fn=r_fn, arg=r_arg, meta_info=expr.meta_info)
                new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, Lambda):
            r_domain = replace_expression_aux(expr.domain, fn, replace_cache, func_name)
            r_body = replace_expression_aux(expr.body, fn, replace_cache, func_name)
            if expr.domain is r_domain and expr.body is r_body:
                new_expr = expr
            else:
                new_expr = Lambda(bname=expr.bname, domain=r_domain, body=r_body, meta_info=expr.meta_info)
                new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, Pi):
            r_domain = replace_expression_aux(expr.domain, fn, replace_cache, func_name)
            r_codomain = replace_expression_aux(expr.codomain, fn, replace_cache, func_name)
            if expr.domain is r_domain and expr.codomain is r_codomain:
                new_expr = expr
            else:
                new_expr = Pi(bname=expr.bname, domain=r_domain, codomain=r_codomain, meta_info=expr.meta_info)
                new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, Let):
            r_domain = replace_expression_aux(expr.domain, fn, replace_cache, func_name)
            r_val = replace_expression_aux(expr.val, fn, replace_cache, func_name)
            r_body = replace_expression_aux(expr.body, fn, replace_cache, func_name)
            if expr.domain is r_domain and expr.val is r_val and expr.body is r_body:
                new_expr = expr
            else:
                new_expr = Let(bname=expr.bname, domain=r_domain, val=r_val, body=r_body, meta_info=expr.meta_info)
                new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, Proj):
            r_expr = replace_expression_aux(expr.expr, fn, replace_cache, func_name)
            if expr.expr is r_expr:
                new_expr = expr
            else:
                new_expr = Proj(sname=expr.sname, index=expr.index, expr=r_expr, meta_info=expr.meta_info)
                new_expr.meta_info.add_func_that_changed(func_name)
...
def replace_expression(
    ...
    It creates a new expression tree, adds meta information from `func`, and propagates MetaInfos.
    """
    return replace_expression_aux(expr, fn, {}, func_name)
...
def replace_expression_w_depth_aux(
    ...
    if new_expr is None:
        if isinstance(expr, BVar):
            new_expr = BVar(db_index=expr.db_index, meta_info=expr.meta_info)
            new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, FVar):
            new_expr = expr  # fvars differentiate by reference
        elif isinstance(expr, Sort):
            new_expr = Sort(level=expr.level, meta_info=expr.meta_info)
            new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, Const):
            new_expr = Const(cname=expr.cname, lvl_params=expr.lvl_params, meta_info=expr.meta_info)
            new_expr.meta_info.add_func_that_changed(func_name)
        elif isinstance(expr, App):
            r_fn = replace_expression_w_depth_aux(expr.fn, fn, depth, replace_cache, func_name)
            r_arg = replace_expression_w_depth_aux(expr.arg, fn, depth, replace_cache, func_name)
            if expr.fn is r_fn and expr.arg is r_arg:
                new_expr = expr
            else:
                new_expr = App(fn=r_fn, arg=r_arg, meta_info=expr.meta_info)
            ...
            r_domain = replace_expression_w_depth_aux(expr.domain, fn, depth, replace_cache, func_name)
            r_body = replace_expression_w_depth_aux(expr.body, fn, depth + 1, replace_cache, func_name)
            if expr.domain is r_domain and expr.body is r_body:
                new_expr = expr
            else:
                new_expr = Lambda(bname=expr.bname, domain=r_domain, body=r_body, meta_info=expr.meta_info)
            ...
            r_domain = replace_expression_w_depth_aux(expr.domain, fn, depth, replace_cache, func_name)
            r_codomain = replace_expression_w_depth_aux(expr.codomain, fn, depth + 1, replace_cache, func_name)
            if expr.domain is r_domain and expr.codomain is r_codomain:
                new_expr = expr 
            else:
                new_expr = Pi(bname=expr.bname, domain=r_domain, codomain=r_codomain, meta_info=expr.meta_info)
            ...
            r_domain = replace_expression_w_depth_aux(expr.domain, fn, depth, replace_cache, func_name)
            r_val = replace_expression_w_depth_aux(expr.val, fn, depth, replace_cache, func_name)
            r_body = replace_expression_w_depth_aux(expr.body, fn, depth + 1, replace_cache, func_name)
            if expr.domain is r_domain and expr.val is r_val and expr.body is r_body:
                new_expr = expr
            else:
            ...
            r_expr = replace_expression_w_depth_aux(expr.expr, fn, depth, replace_cache, func_name)
            if expr.expr is r_expr:
                new_expr = expr
            else:
                new_expr = Proj(sname=expr.sname, index=expr.index, expr=r_expr, meta_info=expr.meta_info)
...
def replace_expression_w_depth(
    expr: Expression, 
    fn: Callable[[Expression, int], Optional[Expression]], 
    depth: int,
    func_name : str
    ...
    It creates a new expression tree, adds meta information from `func`, and propagates MetaInfos.
    """
    return replace_expression_w_depth_aux(expr, fn, depth, {}, func_name)
...
def substitute_level_params_in_expression(
    body: Expression, 
    params: LevelSubList,
    func_name : str
) -> Expression:
    """ Replaces all level parameters in the given expression and adds meta information. """
    def replace_level_params(expr: Expression) -> Optional[Expression]:
        if isinstance(expr, Sort):
            new_expr = Sort(level=substitute_level_params_level(expr.level, params), meta_info=expr.meta_info)
            new_expr.meta_info.add_func_that_changed(func_name)
            return new_expr
        elif isinstance(expr, Const):
            new_expr = Const(cname=expr.cname, lvl_params=[substitute_level_params_level(l, params) for l in expr.lvl_params], meta_info=expr.meta_info)
            new_expr.meta_info.add_func_that_changed(func_name)
            return new_expr
        return None

    return replace_expression(body, replace_level_params, func_name)

def instantiate_bvar(body: Expression, val: Expression, func_name : str) -> Expression:
    """
    Replaces the outermost bound variable in the given expression with the given free variable.
    Throws an error if it finds an unbound bvar index.
    """
    def instantiation_fn(expr: Expression, depth: int) -> Optional[Expression]:
        if not expr.has_loose_bvars:
            return expr
        if isinstance(expr, BVar):
            if expr.db_index == depth:
                return val
        return None
    return replace_expression_w_depth(body, instantiation_fn, 0, func_name)

def instantiate_bvars(body: Expression, vals: Sequence[Expression], func_name : str) -> Expression:
    """
    Replaces the outermost bound variables in the given expression with the given free variables.
    Throws an error if it finds an unbound bvar index.
    """
    def instantiation_fn(expr: Expression, depth: int) -> Optional[Expression]:
        if not expr.has_loose_bvars:
        ...
        if isinstance(expr, BVar):
            if expr.db_index >= depth:
                if expr.db_index - depth < len(vals):
                    return vals[expr.db_index - depth]
                else:
                    raise ValueError(f"Unbound de Bruijn index {expr.db_index}")
        return None
    return replace_expression_w_depth(body, instantiation_fn, 0, func_name)

def abstract_bvar(body: Expression, fvar: FVar, func_name : str) -> Expression:
    ...
    def abstraction_fn(expr: Expression, depth: int) -> Optional[Expression]:
        if not expr.has_fvars:
            return expr
        if isinstance(expr, FVar):
            if expr is fvar:
                new_expr = BVar(db_index=depth, meta_info=expr.meta_info)
                new_expr.meta_info.add_func_that_changed(func_name)
                return new_expr
        return None
    
    return replace_expression_w_depth(body, abstraction_fn, 0, func_name)

def abstract_multiple_bvars(fvars: List[FVar], body: Expression, func_name: str) -> Expression:
    for fvar in fvars:
        assert not fvar.type.has_loose_bvars, f"Cannot abstract a free variable with a loose bound variable in its type: {fvar}"
        if fvar.val is not None:
            assert not fvar.val.has_loose_bvars, f"Cannot abstract a free variable with a loose bound variable in its value: {fvar}"
    ...
    def replace_fn(expr: Expression, depth: int) -> Optional[Expression]:
        ...
        if isinstance(expr, FVar):
            for i, fvar in enumerate(fvars):
                if fvar is expr:
                    new_expr = BVar(db_index=depth + i, meta_info=expr.meta_info)
                    new_expr.meta_info.add_func_that_changed(func_name)
                    return new_expr
        return None
    return replace_expression_w_depth(body, replace_fn, 0, func_name)
...
def level_zip(lvl_params: List[LevelParam], lvl_values: List[Level]) -> LevelSubList:
    ...
    if len(lvl_params) != len(lvl_values):
        raise ValueError("Found different number of level parameters.")
