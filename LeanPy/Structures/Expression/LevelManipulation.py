...
def from_offset(level : Level, offset : int, level_sources : List[Level]) -> Level:
    cur = level
    assert len(level_sources) == offset
...
def make_imax(l : Level, r : Level, source : Level) -> Level:
    if is_not_zero(r):
        return make_max_pair(l, r, source=source.source)
        ...
        return LevelIMax(l, r, source=source.source)
...
def make_max_pair(l1 : Level, l2 : Level, source : Level) -> Level:
    if is_explicit(l1) and is_explicit(l2): return l1 if to_offset(l1)[1] >= to_offset(l2)[1] else l2
        ...
        if structurally_equal(p1[0], p2[0], False):
            ...
            return LevelMax(l1, l2, source=source.source)

def make_max(args : List[Level], sources : List[Level]) -> Level:
    assert len(args) - 1 == len(sources)
    if len(args) == 0: raise PanicLevelError("Empty list passed to make_max")
    if len(args) == 1: return args[0]
    cur = make_max_pair(args[-2], args[-1], sources[-1].source)
    for i in range(len(args) - 3, -1, -1):
        cur = make_max_pair(args[i], cur, sources[i].source)
    return cur
...
def normalize(l : Level) -> Level:
    ...
    if isinstance(r, LevelSucc): raise PanicLevelError("Unreachable code reached in normalize")
        ...
        im = make_imax(normalize(r.lhs), normalize(r.rhs), r.source)
...
def replace_level(level : Level, fn : Callable[[Level], Optional[Level]]) -> Level:
    ...
    elif isinstance(level, LevelSucc): return LevelSucc(replace_level(level.anc, fn), source=level.source)
    elif isinstance(level, LevelMax): return LevelMax(replace_level(level.lhs, fn), replace_level(level.rhs, fn), source=level.source)
    elif isinstance(level, LevelIMax): return LevelIMax(replace_level(level.lhs, fn), replace_level(level.rhs, fn), source=level.source)
