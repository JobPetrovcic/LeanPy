...
class ExpressionToPython:
    ...
    def get_expression_value_str(self, expr : Expression, python_name : str) -> str:
        if isinstance(expr, Sort):
            ...
            return f"Proj(sname={self.get_name(expr.sname)}, index={expr.index}, expr={self.get_expression_value_str(expr.expr, python_name)})"
        else:
    ...
    def get_recursion_rule_value_str(self, rule : RecursorRule, python_name : str) -> str:
        return f"RecursorRule(constructor={self.get_name(rule.constructor)}, num_fields={rule.num_fields}, value={self.get_expression(rule.value, python_name + '_rec_rule')})"
    ...
    def get_declaration_value_str(self, decl : Declaration, python_name : str) -> str:
        if isinstance(decl, Definition):
        ...
        elif isinstance(decl, Inductive):
            inductive_names_str = f"[{', '.join([self.get_name(n) for n in decl.inductive_names])}]"
            constructor_names_str = f"[{', '.join([self.get_name(n) for n in decl.constructor_names])}]"
            return f"Inductive(info={self.get_declaration_info_value_str(decl.info)}, is_recursive={decl.is_recursive}, num_params={decl.num_params}, num_indices={decl.num_indices}, inductive_names={inductive_names_str}, constructor_names={constructor_names_str})"
            ...
            recursor_rules_str = f"[{', '.join([self.get_recursion_rule_value_str(r, python_name + '_rec_rule' + str(i)) for i, r in enumerate(decl.recursor_rules)])}]"
            return f"Recursor(info={self.get_declaration_info_value_str(decl.info)}, num_params={decl.num_params}, num_indices={decl.num_indices}, num_motives={decl.num_motives}, num_minors={decl.num_minors}, recursor_rules={recursor_rules_str}, isK={decl.isK})"
        else:
