...
class LocalContext:
    ...
    def add_fvar(self, fvar : FVar):
        if fvar in self.fvars:
            raise LocalContextError(f"FVar {fvar} already exists in local context")
    ...
    def remove_fvar(self, fvar : FVar):
        ...
        if len(self.fvars) == 0:
            raise LocalContextError(f"Cannot remove FVar {fvar} from empty local context")
        if not (self.fvars[-1] is fvar):
            raise LocalContextError(f"Cannot remove FVar {fvar} from local context: {self.fvars[-1]} is the last FVar")
    ...
    def get_fvar_type(self, fvar : FVar) -> Expression:
        ...
        raise LocalContextError(f"Cannot get type: FVar {fvar.full_identifier()} not found in local context with: {[fvar.full_identifier() for fvar in self.fvars]})")
    ...
    def get_fvar_value(self, fvar : FVar) -> Optional[Expression]:
        ...
        if fvar in self.fvars:
            ...
            if got is None:
                ...
                return got
        raise LocalContextError(f"Cannot get value: FVar {fvar.full_identifier()} not found in local with: {[fvar.full_identifier() for fvar in self.fvars]})")
